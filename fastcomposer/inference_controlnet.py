...
def main():
    ...
    pipe = StableDiffusionControlNetPipeline.from_pretrained(
        args.pretrained_model_name_or_path, safety_checker=None, controlnet=controlnet
    )
    ...
    images_ = output.images
    images = []
    for instance_id in range(args.num_images_per_prompt):
        scaled_data = (images_[instance_id] * 255).astype(np.uint8)
        images.append(Image.fromarray(scaled_data))
